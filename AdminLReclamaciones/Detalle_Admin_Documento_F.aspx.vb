...
Public Class Detalle_Admin_Documento_F
    Inherits System.Web.UI.Page
    Dim obj As New Negocio.NReclamaciones
    Dim objN As New Negocio.NLibro_Reclamacion
    Public Property ruta_documento_fisico As String = String.Empty
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            If Session("user") IsNot Nothing Then
                ...
                Dim id_reclamo_queja As Integer
                id_reclamo_queja = Integer.Parse(ds.Tables(0).Rows(0)(22).ToString())
                Lista_Reclamo_Queja(id_reclamo_queja)
                'lblreclamo_queja.Text = ds.Tables(0).Rows(0)(9).ToString()

                Dim id_producto_servicio As Integer
                id_producto_servicio = Integer.Parse(ds.Tables(0).Rows(0)(23).ToString())
                Lista_Producto_Servicio(id_producto_servicio)
                'lblproducto_servicio.Text = ds.Tables(0).Rows(0)(15).ToString()

                'lblmonto_reclamado.Text = ds.Tables(0).Rows(0)(16).ToString()
                txtmonto_reclamado.Text = ds.Tables(0).Rows(0)(16).ToString()
                'lbldesc_biencontratado.Text = ds.Tables(0).Rows(0)(17).ToString()
                txtdesc_biencontratado.Text = ds.Tables(0).Rows(0)(17).ToString()

                'lbldetalle_reclamo_queja.Text = ds.Tables(0).Rows(0)(10).ToString()
                txtdetalle_reclamo_queja.Text = ds.Tables(0).Rows(0)(10).ToString()

                'lbldetalle_canal_pedido.Text = ds.Tables(0).Rows(0)(11).ToString()
                txtdetalle_canal_pedido.Text = ds.Tables(0).Rows(0)(11).ToString()

                lbldescunidad.Text = ds.Tables(0).Rows(0)(18).ToString()
                'lblnomcliente.Text = ds.Tables(0).Rows(0)(3).ToString()
                txtnomcliente.Text = ds.Tables(0).Rows(0)(3).ToString()

                txtdnicliente.Text = ds.Tables(0).Rows(0)(5).ToString()
                txtdireccioncliente.Text = ds.Tables(0).Rows(0)(4).ToString()
                Dim email As String
                email = ds.Tables(0).Rows(0)(7).ToString()
                lblemail_cliente.Text = email
                txttelefono_email.Text = ds.Tables(0).Rows(0)(6).ToString() '+ "/" + email
                txtemail.Text = email
                ...
                If id_estado <> 1 And id_estado <> 7 Then
                ...
                nom_archivo = ds.Tables(0).Rows(0)(24)
                ruta_documento_fisico = "Adjuntos_Cliente/" + nom_archivo '"assets/images/big/foto_ejemplo.jpg"

                'OJOOOOOOOO!!!!! hay que aparecer la imagen
                img_documento_fisico.ImageUrl = "~/" + "Adjuntos_Cliente/" + nom_archivo
                lblnombreimagen.Text = nom_archivo


                txtsustentoanulacion.Text = ds.Tables(0).Rows(0)(25).ToString()

                Documento_Anulado_Fisico(id_estado)
    ...
    End Sub
    Public Sub Documento_Anulado_Fisico(ByVal id_estado As Integer)
        Dim id_perfil As String
        ...
        If id_estado = 7 Then
            ...
            chkanulado_fisico.Checked = True
            txtsustentoanulacion.Visible = True
            pnleditables1.Enabled = False
            pnleditables2.Enabled = False
        End If
        If id_estado = 2 Then
            chkanulado_fisico.Enabled = False
            pnleditables1.Enabled = False
            pnleditables2.Enabled = False
        End If
        If id_estado = 3 Then
        ...
        'End If
    End Sub
    Public Sub Lista_Reclamo_Queja(ByVal id_reclamo_queja As Integer)
        Dim ds As New DataSet
        ds = objN.Lista_Reclamo_Queja()
        ...
        cboquejareclamo.SelectedValue = id_reclamo_queja
        ds.Dispose()
    End Sub
    Public Sub Lista_Producto_Servicio(ByVal id_producto_servicio As Integer)
        Dim ds As New DataSet
        ds = objN.Lista_Producto_Servicio()
    ...
    Protected Sub btnaprobar_Click(sender As Object, e As EventArgs) Handles btnaprobar.Click
        ...
        If chkanulado_fisico.Checked = True Then
            'Cambia a estado "Anulado - Físico" y no guarda cambio alguno
            'Inicio - Validacion de los nuevos campos actualizables

            Dim tipo_documentox As Integer
            Dim producto_serviciox As Integer
            Dim monto_reclamadox As String
            Dim desc_bien_contratadox As String
            Dim detalle_reclamacionx As String
            Dim Pedido_clientex As String

            tipo_documentox = Integer.Parse(cboquejareclamo.SelectedValue.ToString())
            producto_serviciox = Integer.Parse(cboproducto_servicio.SelectedValue.ToString())
            monto_reclamadox = txtmonto_reclamado.Text.ToString()
            desc_bien_contratadox = txtdesc_biencontratado.Text.ToString()
            detalle_reclamacionx = txtdetalle_reclamo_queja.Text.ToString()
            Pedido_clientex = txtdetalle_canal_pedido.Text.ToString()


            If cboquejareclamo.SelectedValue = 0 Then
                lblmensaje_validacion.Text = "Debe seleccionar si el documento es una queja o reclamo."
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "modal-validacion", "$('#modal-validacion').modal();", True)
                Return
            End If
            If cboproducto_servicio.SelectedValue = 0 Then
                lblmensaje_validacion.Text = "Debe seleccionar si es un producto o servicio."
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "modal-validacion", "$('#modal-validacion').modal();", True)
                Return
            End If
            If detalle_reclamacionx = "" Then
                lblmensaje_validacion.Text = "Debe ingresar el detalle del reclamo."
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "modal-validacion", "$('#modal-validacion').modal();", True)
                Return
            End If
            If Pedido_clientex = "" Then
                lblmensaje_validacion.Text = "Debe ingresar el pedido del cliente."
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "modal-validacion", "$('#modal-validacion').modal();", True)
                Return
            End If
            If txtnomcliente.Text = "" Then
                lblmensaje_validacion.Text = "Debe ingresar el nombre del cliente."
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "modal-validacion", "$('#modal-validacion').modal();", True)
                Return
            End If
            If txtdnicliente.Text = "" Then
                lblmensaje_validacion.Text = "Debe ingresar el DNI del cliente O Documento de Identidad."
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "modal-validacion", "$('#modal-validacion').modal();", True)
                Return
            End If
            If txtdireccioncliente.Text = "" Then
                lblmensaje_validacion.Text = "Debe ingresar la dirección del cliente."
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "modal-validacion", "$('#modal-validacion').modal();", True)
                Return
            End If
            If txttelefono_email.Text = "" Then
                lblmensaje_validacion.Text = "Debe ingresar el teléfono del cliente."
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "modal-validacion", "$('#modal-validacion').modal();", True)
                Return
            End If
            If txtemail.Text = "" Then
                lblmensaje_validacion.Text = "Debe ingresar el Email del cliente."
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "modal-validacion", "$('#modal-validacion').modal();", True)
                Return
            End If
            If txtsustentoanulacion.Text = "" Then
                lblmensaje_validacion.Text = "El documento al ser anulado, debe ingresar un sustento."
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "modal-validacion", "$('#modal-validacion').modal();", True)
                Return
            End If
            'Fin - Validacion de los nuevos campos actualizables
            'Fin - Validacion de los nuevos campos actualizables

            'Dim id_documento_flujo As Integer
            'id_documento_flujo = obj.Registra_Documento_flujo(lblnrodocumento.Text.ToString(), user_ad, detalle_accion,
            '                                     costo_accion, obs_legal, obs_gerente_marca, id_perfil,
            '                                      aprobado, otros_gastos, atencion_manual)


            obj.Actualizar_Documento_campos_fisico(lblnrodocumento.Text.ToString(), tipo_documentox, producto_serviciox, monto_reclamadox, desc_bien_contratadox, detalle_reclamacionx, Pedido_clientex,
                                                    txtnomcliente.Text, txtdnicliente.Text, txtdireccioncliente.Text, txttelefono_email.Text, txtemail.Text)

            'Estado anulado
            Dim sustento_anulacion As String
            sustento_anulacion = txtsustentoanulacion.Text.ToString()
            obj.Anular_Documento_fisico(lblnrodocumento.Text.ToString(), sustento_anulacion) 'Se anula el documento cambiando de estado en documento_admin
            Response.Redirect("~/admin_documentos.aspx")

        Else
            'Proceso Normal + la actualizacion de los campos.
    ...
    Public Sub Registro_Aprobar_Rechazar(ByVal aprobado As Integer)
        ...
        obj.Actualizar_Documento_campos_fisico(nro_documento, tipo_documentox, producto_serviciox, monto_reclamadox, desc_bien_contratadox, detalle_reclamacionx, Pedido_clientex,
                                                txtnomcliente.Text, txtdnicliente.Text, txtdireccioncliente.Text, txttelefono_email.Text, txtemail.Text)
    ...
    Public Sub Generar_PDF(ByVal carta_cliente As String)
        ...
        Dim cell_tb03_01 As New PdfPCell(New iTextSharp.text.Phrase(txtnomcliente.Text, New Font(Font.FontFamily.TIMES_ROMAN, 12.0F, Font.NORMAL)))
        ...
        Dim cell_tb04_01 As New PdfPCell(New iTextSharp.text.Phrase(txtdireccioncliente.Text, New Font(Font.FontFamily.TIMES_ROMAN, 12.0F, Font.NORMAL)))
    ...
    Public Sub Enviar_Correo_Cambio_Estado()
        ...
        Nro_Documento = lblnrodocumento.Text.ToString()
        Nom_Cliente = txtnomcliente.Text.ToString()
        dni = txtdnicliente.Text.ToString()
    ...
    '        Dim oAttachment As Attachment = oMail.AddAttachment(ImagenCorreo1)
    '        Dim contentID As String = "test001@host"
    '        oAttachment.ContentID = contentID

    '        Dim oAttachment2 As Attachment = oMail.AddAttachment(ImagenCorreo2)
    '        Dim contentID2 As String = "test002@host"
    ...
    '                <tr style=" + """" + "background-color: #a8123e;" + """" + ">
    '                <td style=" + """" + "width 696px;" + """" + " colspan=" + """" + "2" + """" + ">
    '                <p><img src=""cid:" & contentID & """></p>
    '                </td>
    '                </tr>
    '                <tr style=" + """" + "background-color: #a8123e;" + """" + ">
    '                <td style=" + """" + "width 696px;" + """" + " colspan=" + """" + "2" + """" + ">
    '                <p><img src=""cid:" & contentID2 & """></p>
    '                </td>
    '                </tr>
    '                <tr>
    '                <td style=" + """" + "width 696px;" + """" + " colspan=" + """" + "2" + """" + ">
    ...
    '                <td style=" + """" + "width 696px;" + """" + " colspan=" + """" + "2" + """" + ">
    '                <p style=" + """" + "padding-left: 30px;" + """" + "><strong>Días Transcurridos desde la creación del documento: " + dias_transcurridos.ToString() + " días.</strong></p>
    '                <p style=" + """" + "padding-left: 30px;" + """" + "><strong>¡Tener en cuenta que, a partir del 22.05.2022 por cambio normativo el tiempo de respuesta se reducirá a 15 días hábiles improrrogables!</strong></p>
    '                </td>
    '                </tr>

    '                <tr>
    '                <td style=" + """" + "width 696px;" + """" + " colspan=" + """" + "2" + """" + ">
    '                <p style=" + """" + "padding-left: 30px;" + """" + "><strong><a href=" + """" + "http://10.0.0.3/areclamos" + """" + ">Click para acceder al Sistema</a></strong></p>
    '                </td>
    '                </tr>
    ...
    '            email_responsable = ds_contactos.Tables(0).Rows(j)(0).ToString
    '            'email_responsable = "imartinez@acuriorestaurantes.com"
    '            oMail.To.Add(email_responsable)
    '        Next
    ...
    '        Dim oServer As SmtpServer = New SmtpServer("smtp.office365.com")
    '        oServer.User = EmailFrom
    '        oServer.Password = PassFrom
    '        oServer.Port = 587
    ...
    Protected Sub chklistAccion_SelectedIndexChanged(sender As Object, e As EventArgs) Handles chklistAccion.SelectedIndexChanged
        'Dim check_cortesia As String
        'check_cortesia = chklistAccion.SelectedValue.ToString()

        'If check_cortesia = "5" Then 'Selecciono Cortesias
        '    pnlcortesias.Visible = True
        '    Return
    ...
    Protected Sub btnDescargarImagen_Click(sender As Object, e As EventArgs) Handles btnDescargarImagen.Click
        Dim IPSERVER As String = ConfigurationManager.AppSettings("Adjuntos_Cliente")
        Dim ruta As String
        Dim nombre_archivo As String
        nombre_archivo = lblnombreimagen.Text
        'nro_ruc = lblruc_prov.Text.ToString()
    ...
    Public Sub registro_acceso_pagina(ByVal TokenGlobal As String, ByVal sistema As String, ByVal user As String)
        obj.Registro_SesionSistema(TokenGlobal, user, sistema, "Detalle_Admin_Documento_F.aspx")
    End Sub
